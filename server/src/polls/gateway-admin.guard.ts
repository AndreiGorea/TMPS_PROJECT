...
export class LoggingGatewayAdminGuardDecorator implements CanActivate {
  private readonly logger = new Logger(LoggingGatewayAdminGuardDecorator.name);

  constructor(private readonly guard: GatewayAdminGuard) {}

  async canActivate(context: ExecutionContext): Promise<boolean> {
    this.logger.debug('Before executing GatewayAdminGuard.');

    const result = await this.guard.canActivate(context);

    this.logger.debug('After executing GatewayAdminGuard.');
